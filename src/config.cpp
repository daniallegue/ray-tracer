...
static std::ostream& operator<<(std::ostream& os, const SceneType& sceneType)
{
    ...
    case SceneType::CornellBoxTransparency: {
        os << "SceneType::CornellBoxTransparency";
        break;
    }
...
std::ostream& operator<<(std::ostream& os, const Config& config)
{
    ...
    os << "  + output_filepath: " << config.outputDir << std::endl
       ...
       << "    - enable_reflections: " << config.features.enableReflections << std::endl
       << "    - enable_shadows: " << config.features.enableShadows << std::endl
       ...
       << "    - enable_transparency: " << config.features.enableTransparency << std::endl
       << "    - shading_model: " << static_cast<uint32_t>(config.features.shadingModel) << std::endl
       << "    - num_pixel_samples: " << config.features.numPixelSamples << std::endl
    ...
    os << "    - enable_environment_map: " << config.features.extra.enableEnvironmentMap << std::endl;
    ...
    os << "    - enable_bilinear_texture_filtering: " << config.features.enableBilinearTextureFiltering << std::endl;
...
Config readConfigFile(const std::filesystem::path& config_path)
{
    ...
    config.windowSize = tomlArrayToIVec2(table["window_size"].as_array()).value_or(glm::ivec2(800, 800));
    ...
    config.features.enableShading = table["features"]["enable_shading"]
                                        .as_boolean()
                                        ->value_or(false);
    config.features.enableReflections = table["features"]["enable_reflections"]
                                            .as_boolean()
                                            ->value_or(false);
    config.features.enableShadows = table["features"]["enable_shadows"]
                                        .as_boolean()
                                        ->value_or(false);
    ...
    config.features.enableAccelStructure = table["features"]["enable_accel_structure"]
                                               .as_boolean()
                                               ->value_or(false);
    config.features.numPixelSamples = table["features"]["num_pixel_samples"]
                                          .as_integer()
                                          ->value_or(1);
    config.features.shadingModel = static_cast<ShadingModel>(table["features"]["shading_model"]
                                                                 .as_integer()
                                                                 ->value_or(0));
    ...
    if (table["features"]["extra"]["enable_multiple_rays_per_pixel"]) {
        config.features.enableJitteredSampling = table["features"]["enable_jittered_sampling"]
                                                           .as_boolean()
                                                           ->value_or(false);
    }
...
std::string serialize(const SceneType& sceneType)
{
    ...
    case SceneType::CornellBox:
        return "cornell_box";
    case SceneType::CornellBoxTransparency:
        return "cornell_box_transparency";
    case SceneType::CornellBoxParallelogramLight:
...
{
    std::string lowered;
    std::transform(std::begin(sceneTypeStr), std::end(sceneTypeStr), std::back_inserter(lowered), [](const char c) { return (char)::tolower(c); });
    ...
    } else if (lowered == "cornell_box_transparency" || lowered == "cornellboxtransparency" || lowered == "cornell-box-transparency") {
        return SceneType::CornellBoxTransparency;
