...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int i = 0; i < numSamples; i++) {
        glm::vec2 position = (glm::vec2(pixel) + state.sampler.next_2d() - glm::vec2(0.5f)) / glm::vec2(screenResolution) * 2.f - 1.f;
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int p = 0; p < n; p++) {
        for (int q = 0; q < n; q++) {
            ...
            glm::vec2 position = { i - 0.5, j - 0.5};
            Ray r = camera.generateRay(position / glm::vec2(screenResolution) * 2.0f - 1.0f);
            rays.push_back(r);
        }
    }
    return rays;
